...
class SimpleDataSet(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            print(f"Attempting to open: {img_path} (from line: {data_line.strip()})")
            with open(data["img_path"], "rb") as f:
                img = f.read()
                data["image"] = img
            ...
            print(f"Transforms applied successfully for: {img_path}")
            
        except Exception as e:
            error_msg = f"Error at idx {idx} (file_idx {file_idx}): {e} - Data line: {data_line.strip()}"
            print(error_msg)
            ...
            return self.__getitem__(rnd_idx)  # Giữ retry nhưng sẽ thêm giới hạn ở bước sau
        
        if outs is None:
            print(f"Transform returned None for: {img_path}")
            rnd_idx = (
                np.random.randint(self.__len__())
                if self.mode == "train"
                else (idx + 1) % self.__len__()
